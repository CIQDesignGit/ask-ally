...
| 2026-09-24 | `GapToPlanReport` header KPIs | Equal gap / actual / attainment figures in the card header (same type, hairline dividers). Semantic color only on gap. Plan vs actual keeps the week bars. | Header had unused right-side space; the featured last-week bar was repeating the same chart. | `Local workaround` | [`GapToPlanReport.tsx`](../src/components/answer/GapToPlanReport.tsx) `HeaderVerdict` | — |
| 2026-09-24 | Driver flowchart | Display-only React Flow (`@xyflow/react`) recipe: custom compare + metric nodes, step edges, no pan/zoom/drag. | Ecommerce equation breakdown needs a real parent→children flow, not CSS-drawn connectors. | `Local workaround` | [`DriverFlow.tsx`](../src/components/answer/DriverFlow.tsx) | — |
